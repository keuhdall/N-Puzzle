module Solver.Solver (SearchType(..), readSearchType, solve) where
    ...
    runSearch goal xss os cs nn n
        ...
        | suc /= PQ.empty                                       = displayGrid xs >> runSearch   goal   ((minim suc):xss)    (PQ.union os $ PQ.deleteMin suc)             cs' nn (n+1)
        | suc == PQ.empty && os' /= PQ.empty                    = displayGrid xs >> runSearch   goal   ((minim os'):xss)    (PQ.filter (/= (snd $ PQ.findMin os')) os)   cs' nn (n+1)
